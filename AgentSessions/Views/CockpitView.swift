...
struct CockpitView: View {
    ...
    private func makeLiveRowsSnapshot() -> LiveRowsSnapshot {
        ...
        let mapped: [Row] = activeCodex.presences.compactMap { p in
            ...
            let title = session?.title
                ?? p.sessionId.map { String(format: String(localized: "Session %@"), String($0.prefix(8))) }
                ?? String(format: String(localized: "Active %@ session"), p.source.displayName)
            ...
            let terminal: String = {
                ...
                if termProgram.lowercased().contains("terminal") { return String(localized: "Terminal") }
            ...
            let focusHelp: String = {
                if CodexActiveSessionsModel.canAttemptITerm2Focus(
                    ...
                    return String(localized: "Focus the existing iTerm2 tab/window for this session.")
                }
                return String(localized: "Focus is unavailable for this terminal session.")
            }()
    ...
    private func sourceLabel(for source: SessionSource) -> String {
        ...
        case .copilot: return String(localized: "Copilot")
