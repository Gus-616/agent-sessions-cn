...
enum CodexImagesScope: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .singleSession:
            return String(localized: "Single Session")
        case .project:
            return String(localized: "All Sessions in Project")
        }
    }
}
...
struct CodexSessionImagesGalleryView: View {
            ...
            Menu {
                Button {
                    ...
                    if viewModel.selectedProject == nil {
                        Label(String(localized: "All Projects"), systemImage: "checkmark")
                    } else {
                        Text(String(localized: "All Projects"))
                    }
                }
                ...
                HStack(spacing: 6) {
                    Text(viewModel.selectedProject ?? String(localized: "All Projects"))
                        .lineLimit(1)
                        .truncationMode(.tail)
            ...
            Menu {
                Button {
                    ...
                    agentMenuCheckboxRow(
                        title: String(localized: "All Coding Agents"),
    ...
    private var agentMenuTitle: String {
        ...
        if viewModel.isAllCodingAgentsSelected { return String(localized: "All Coding Agents") }
        if selected.count == 1, let one = selected.first { return one.displayName }
        if selected.isEmpty { return String(localized: "No Agents") }
        return String(format: String(localized: "%lld selected"), selected.count)
    }
    ...
    private var content: some View {
        ...
        case .indexingBackground, .loaded:
            if viewModel.items.isEmpty {
                ContentUnavailableView(String(localized: "No images found"), systemImage: "photo")
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Color(NSColor.textBackgroundColor))
            } else {
                HSplitView {
    ...
    private var loadingStatusText: String {
        ...
        case .loadingSelected:
            return String(localized: "Indexing selected session…")
        case .indexingBackground:
            return String(localized: "Indexing images…")
        case .failed:
            return String(localized: "Index failed.")
        case .idle, .loaded:
            ...
            if let item = selectedItem {
                ZStack {
                    ...
                    if isPreviewLoading {
                        ...
                        ContentUnavailableView(String(localized: "Select an image"), systemImage: "photo")
                            .padding(12)
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack(spacing: 10) {
                        Button(String(localized: "Navigate to Session")) { navigateToSession(item: item) }
                        ...
                        .buttonStyle(.borderless)
                        .help(String(localized: "Copy Image Path (for CLI agent)"))
                        ...
                        .buttonStyle(.borderless)
                        .help(String(localized: "Copy Image"))
                        ...
                        .buttonStyle(.borderless)
                        .disabled(isSaving)
                        .help(String(localized: "Save to Downloads"))

                        Button(isSaving ? String(localized: "Saving…") : String(localized: "Save…")) { saveWithPanel(item: item) }
                            .disabled(isSaving)
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(String(localized: "USER PROMPT"))
                            .font(.system(size: 11, weight: .semibold, design: .monospaced))
                            .foregroundStyle(.secondary)

                        Text(promptText?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false ? (promptText ?? "") : String(localized: "Prompt unavailable"))
                            .font(.system(size: 12, weight: .regular, design: .monospaced))
                            .foregroundStyle(.primary)
                ...
                ContentUnavailableView(String(localized: "Select an image"), systemImage: "photo.on.rectangle")
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
            }
        }
        .padding(12)
    ...
    private func imageActionsMenu(for item: ImageBrowserViewModel.Item) -> some View {
        Button(String(localized: "Open in Preview")) { openInPreview(item: item) }

        Divider()

        Button(String(localized: "Copy Image Path (for CLI agent)")) { copyImagePath(item: item) }
        Button(String(localized: "Copy Image")) { copyImage(item: item) }

        Divider()

        Button(String(localized: "Save to Downloads")) { saveToDownloads(item: item) }
        Button(String(localized: "Save…")) { saveWithPanel(item: item) }
    }
