...
struct AgentCockpitHUDView: View {
    ...
    private func rowContextMenu(_ row: HUDRow) -> some View {
        Button(String(localized: "Go to Session")) {
            goToSession(row)
        }
        .disabled(!activeEnabled || !row.navigationConfidence.isNavigable)
        .help(row.navigationConfidence.isNavigable
              ? String(localized: "Select this session in the main Agent Sessions window and open its transcript.")
              : String(localized: "No exact session match found — only a working-directory fallback is available."))

        Button(String(localized: "Focus in iTerm2")) {
            focus(row)
        }
        .disabled(!activeEnabled || !canFocus(row))
        .help(String(localized: "Focus the existing iTerm2 tab/window for this session."))
