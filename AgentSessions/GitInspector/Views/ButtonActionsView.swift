...
struct ButtonActionsView: View {
    ...
    private func openDirectory() {
        ...
        if FileManager.default.fileExists(atPath: cwd, isDirectory: &isDirectory), isDirectory.boolValue {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "Directory Not Found")
            alert.informativeText = String(format: String(localized: "The directory no longer exists:\n%@"), cwd)
    ...
    private var resumeButtonLabel: String {
        ...
        case .safe: return String(localized: "Resume Session")
        case .caution, .warning: return String(localized: "Resume Anyway")
        case .unknown: return String(localized: "Resume")
        }
    }
    ...
    private var resumeButtonHelp: String {
        ...
        switch safety.status {
        case .safe: return String(localized: "Safe to resume")
        case .caution: return String(localized: "Uncommitted changes detected - resume with caution")
        case .warning: return String(localized: "Git state changed - resume with caution")
        case .unknown: return String(localized: "Unable to verify safety")
        }
    }
    ...
    private func handleResume() {
        ...
        if safety.shouldWarnBeforeResume {
            let alert = NSAlert()
            alert.messageText = safety.status == .caution ? String(localized: "Uncommitted Changes Detected") : String(localized: "Git State Changed")
            alert.informativeText = safety.recommendation
            alert.alertStyle = .warning
            alert.addButton(withTitle: String(localized: "Resume Anyway"))
