...
enum AnalyticsAggregationMetric: String, CaseIterable, Identifiable {
    ...
    var detailDescription: String {
        switch self {
        case .sessions: return String(localized: "Counts unique conversations")
        case .messages: return String(localized: "Counts total messages across conversations")
        }
    }
...
struct AnalyticsAgentBreakdown: Identifiable, Equatable {
    ...
    func details(for metric: AnalyticsAggregationMetric) -> String {
        switch metric {
        case .sessions:
            let suffix = sessionCount == 1 ? String(localized: "session") : String(localized: "sessions")
            return "\(AnalyticsSummary.formatNumber(sessionCount)) \(suffix) • \(durationFormatted)"
        case .messages:
            return "\(AnalyticsSummary.formatNumber(messageCount)) \(String(localized: "messages")) • \(durationFormatted)"
        }
    }
